...
RUN groupadd -r LLOneBot && useradd -r -g LLOneBot LLOneBot && \
    ...
    curl -L -o /tmp/LLOneBot.zip https://github.com/linyuchen/LiteLoaderQQNT-OneBotApi/releases/download/$(curl -Ls "https://api.github.com/repos/linyuchen/LiteLoaderQQNT-OneBotApi/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')/LLOneBot.zip && \
